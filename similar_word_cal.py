...
class CharFuncs(object):
    def __init__(self, cfg,data):
        # self.data = self.load_char_meta(cfg.char_meta_fname)
        self.data=data
        self.char_dict = dict([(c, 0) for c in self.data])
        self.threshold_shape = cfg.threshold_shape
    ...
    def is_ShapeSimilar(self,ori_word,fix_word):
        rank = self.shape_similarity(char1=ori_word,char2=fix_word)
        if rank >= self.threshold_shape:
            return True
        else:
            return False
